...
function ListingsContent() {
  ...
  return (
    <div className="min-h-screen bg-[#F2F7F7]">
      {/* Page header band */}
      <div className="relative overflow-hidden bg-[linear-gradient(160deg,#66C1B4_0%,#00433C_60%,#003830_100%)] pt-28 pb-20">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 pb-8">
        ...
        <div className="relative z-10 -mt-10 flex flex-col sm:flex-row gap-3 mb-6 bg-white rounded-2xl ring-1 ring-black/5 shadow-[0_16px_40px_-16px_rgba(0,40,35,0.25)] p-3">
          <div className="relative flex-1 min-w-0 rounded-xl bg-gray-100/80 focus-within:bg-white focus-within:ring-2 focus-within:ring-[#66C1B4]/25 transition-all duration-200">
          ...
          <div className="flex gap-3">
            ...
            Filtro
            {hasActiveFilters && (
              <span className="ml-2 bg-[#66C1B4] text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">
          ...
          <div className="bg-white ring-1 ring-black/5 rounded-2xl p-5 mb-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 2xl:grid-cols-7 min-[2000px]:grid-cols-9 min-[3000px]:grid-cols-12 gap-4">
                ...
                {cityOpen && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200/80 rounded-xl shadow-[0_12px_32px_-8px_rgba(0,40,35,0.18)] z-50 p-1 max-h-60 overflow-y-auto">
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors rounded-lg ${!filters.city ? 'text-[#4CA195] bg-[#66C1B4]/10' : 'text-[#1A1A2E] hover:bg-gray-50'}`}
                    >
                      Të gjitha
                    </button>
                    {CITIES.map(city => (
                      <button
                        ...
                        type="button"
                        onClick={() => { setFilters(prev => ({ ...prev, city, neighborhood: '' })); setCityOpen(false) }}
                        className={`w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors rounded-lg ${filters.city === city ? 'text-[#4CA195] bg-[#66C1B4]/10' : 'text-[#1A1A2E] hover:bg-gray-50'}`}
                      >
                        {city}
                      </button>
                    ))}
                    ...
                    {neighborhoodOpen && (
                      <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200/80 rounded-xl shadow-[0_12px_32px_-8px_rgba(0,40,35,0.18)] z-50 p-1 max-h-60 overflow-y-auto">
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors rounded-lg ${!filters.neighborhood ? 'text-[#4CA195] bg-[#66C1B4]/10' : 'text-[#1A1A2E] hover:bg-gray-50'}`}
                        >
                          Të gjitha lagjet
                        </button>
                        {(KOSOVO_LOCATIONS[filters.city] || []).map(neighborhood => (
                          <button
                            ...
                            type="button"
                            onClick={() => { setFilters(prev => ({ ...prev, neighborhood })); setNeighborhoodOpen(false) }}
                            className={`w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors rounded-lg ${filters.neighborhood === neighborhood ? 'text-[#4CA195] bg-[#66C1B4]/10' : 'text-[#1A1A2E] hover:bg-gray-50'}`}
                          >
                            {neighborhood}
                          </button>
                        ))}
                ...
                {typeOpen && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200/80 rounded-xl shadow-[0_12px_32px_-8px_rgba(0,40,35,0.18)] z-50 p-1">
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors rounded-lg ${!filters.type ? 'text-[#4CA195] bg-[#66C1B4]/10' : 'text-[#1A1A2E] hover:bg-gray-50'}`}
                    >
                      Të gjitha
                    </button>
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors rounded-lg ${filters.type === 'shitje' ? 'text-[#4CA195] bg-[#66C1B4]/10' : 'text-[#1A1A2E] hover:bg-gray-50'}`}
                    >
                      Shitje
                    </button>
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors rounded-lg ${filters.type === 'qira' ? 'text-[#4CA195] bg-[#66C1B4]/10' : 'text-[#1A1A2E] hover:bg-gray-50'}`}
                    >
                      Me qira
                    </button>
                  </div>
            ...
            <div>
              ...
              <input
                ...
                type="number"
                placeholder="0"
                className="w-full h-11 px-3.5 bg-gray-50 border border-gray-200/80 hover:border-gray-300 text-[#1A1A2E] placeholder:text-gray-400 rounded-xl text-sm focus:bg-white focus:border-[#66C1B4]/50 focus:outline-none transition-colors"
                ...
                {roomsOpen && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200/80 rounded-xl shadow-[0_12px_32px_-8px_rgba(0,40,35,0.18)] z-50 p-1">
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors rounded-lg ${!filters.rooms ? 'text-[#4CA195] bg-[#66C1B4]/10' : 'text-[#1A1A2E] hover:bg-gray-50'}`}
                    >
                      Të gjitha
                    </button>
                    {[1,2,3,4,5].map(r => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors rounded-lg ${filters.rooms === String(r) ? 'text-[#4CA195] bg-[#66C1B4]/10' : 'text-[#1A1A2E] hover:bg-gray-50'}`}
                      >
                        {r}+
                      </button>
                    ))}
                  ...
                  <div className="absolute top-full right-0 mt-2 bg-white border border-gray-200/80 rounded-xl shadow-[0_12px_32px_-8px_rgba(0,40,35,0.18)] z-50 p-1 min-w-[160px] overflow-hidden">
                    {([
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors rounded-lg ${sortBy === opt.value ? 'text-[#4CA195] bg-[#66C1B4]/10' : 'text-[#1A1A2E] hover:bg-gray-50'}`}
                      >
                        {opt.label}
                      </button>
                    ))}
